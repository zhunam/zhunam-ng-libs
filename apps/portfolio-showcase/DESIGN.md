...
- **Shape:** 0.75rem radius, 1px Hairline border, no shadow at rest. Laid out in a real CSS grid (`grid-cols-1` mobile, `sm:grid-cols-2`, `lg:grid-cols-3`, `xl:grid-cols-4`), not `flex-wrap`: grid rows give every card in a row identical height for free (CSS Grid's own default `align-items: stretch`), and the extra `xl:grid-cols-4` step uses the wide-desktop width that used to sit empty next to a 3-column row. No `max-w-4xl` cap on the grid itself anymore, the column count is what adapts to available width, not a fixed container cap. Superseded: an earlier draft added a small abstract thumbnail graphic below each card's description; user feedback confirmed the cards read better without it, so it was removed with no replacement, not swapped for another graphic.
- **Icon badge (new):** each real card (not the "coming soon" placeholder, which already had its own 4rem badge, see below) leads with a small icon: same treatment already used by the placeholder variant, a circular Signal-Teal-tinted badge (`bg-primary/10`, icon in solid Signal Teal), scaled down to fit inline above the library name (2.5rem badge, 1.25rem icon) rather than the placeholder's larger centered 4rem/2rem version. One hand-drawn inline SVG glyph per library, chosen to read as that library's domain at a glance (e.g. a grid/table glyph for Data Grid), never a stock icon set.
- **Button anchored to the bottom:** each card is `flex flex-col`; the "View full demo" button carries `mt-auto` so it sits flush with the card's bottom edge regardless of how much description/checkmark text sits above it. Combined with the grid's own equal-row-height behavior, every button in a row lands on the same horizontal line even when card text length varies.
...
- **Shape (revised, two columns):** one wide bounded panel (0.75rem radius, 1px Hairline border, no shadow) that fills the section's own row, split into a two-column grid at `lg` and up (stacks to one column below that): a text column (Live badge, title, description, "View full demo" button) on the left, and a dark preview panel on the right showing a stylized, simplified miniature of the real dashboard (a small price strip, a mini sparkline, a 2-row mini price list). One project at a time; if a second featured project is ever added, design it as a real multi-card grid at that point instead of forcing this shape to hold two.
- **The whole card stays one click target.** The entire panel (both columns) is a single `routerLink` anchor, same as the original one-column version; "View full demo" is a `.btn`-styled element inside that same anchor, not a second nested link (invalid HTML). This keeps existing hover/focus behavior and existing tests (`a[href="/crypto-dashboard"] .badge-primary`) unchanged.
...
- **Dark preview panel (new):** same Ink surface (`bg-base-content`) as the crypto-dashboard hero and the home hero, `rounded-box` on its own (it doesn't span full-bleed, it's confined to its grid column). Content is fixed sample data for the miniature, not a live fetch or an iframe of the real page — a decorative representation of the page's shape, not a second working instance of it. State this plainly in a code comment where it's built, so it never gets mistaken for "fabricating functionality," per Do's and Don'ts: it's a graphic, not something pretending to function.
...
- **Never:** a gradient, a second accent color, or a `bg-slate-100`-style fill to make it stand out from Library Explorer cards. The size difference, the Live badge, and now the dark preview panel already do that job inside the existing system.
...
### Tabs
- **Use:** switching which dataset a single, otherwise-unchanged component displays (first used on `/crypto-dashboard`'s Trending section: "Trending" vs. "Top Movers", both rendered by the same `trending-carousel`, only its `coins`/`error`/retry inputs swap). Not a navigation pattern, not for switching pages or routes, only for toggling the data source feeding one view in place.
- **Style (revised after visual review):** pill-shaped like Badges, but **raised surface**, not bordered — per the Ghost-Card Refusal, a border and a shadow never pair on the same element, so this picks the shadow side of that choice rather than softening the border. Both tabs share the same soft shadow (`shadow-[0_2px_10px_rgba(15,37,42,0.08)]`, the identical value this page's own panels already use), no border on either state. The active tab fills Signal Teal (`bg-primary`, white text, same hue as the Live badge); the inactive tab fills Vellum (`bg-base-100`, Ink Muted text) — the shadow alone defines its edge against the page background. The first draft used `badge-outline` (a Hairline-bordered pill, matching Coming Soon) for the inactive state; in practice its border read as too heavy for a frequently-clicked control, so it was replaced with this shadow-only treatment instead of tuning the border's weight.
- **Placement:** sits directly above the component whose content it controls, left-aligned, `gap-2` between tabs.
- **Never:** don't fetch new data just to populate a second tab if the page already has an equivalent dataset loaded for another purpose. "Top Movers" reuses the same top-N `market-table` coins already fetched, sorted client-side by `|24h change|`, not a second API call.

### Price Direction Indicator
- **Use:** any percentage or value change tied to real market data (24h price change, market cap change), first introduced in `price-ticker` (crypto-dashboard). That component was later deleted as dead code (its role fully absorbed by the hero during a redesign), but the convention it introduced didn't go with it: still used by `market-table`, `trending-carousel`, `market-ticker`, `market-state`, and the hero.
...
- **Shared component:** this whole pattern (checkbox, mobile trigger tab, drawer, sidebar `<aside>`) is `LibraryPageShell` (`apps/portfolio-showcase/src/app/shared/library-page-shell/`), not duplicated per page. Each demo page projects its own page-specific content (breadcrumb, header, the actual library demo, its own Usage panel) via `<ng-content>` into the shell; the shell alone owns the sidebar markup, the current-route highlighting, and (see below) the package info card. `/crypto-dashboard` doesn't use this shell at all — it isn't a library, has no Library Explorer sidebar, and (per its own page-specific register above) no breadcrumb either.
- **Sidebar sticky offset (fixed):** the `<aside>` uses `lg:sticky lg:top-16` — `top-16` (4rem), matching the sticky header's own `h-16` exactly (see Sticky Header below), not an arbitrary smaller value. A smaller offset here previously let the header cover the top ~40px of the sidebar on scroll; `top-16` is the header's real height, not a guess.
- **Package info card (new):** sits directly below the sidebar's nav list, inside the same `<aside>`. Compact, real data only, driven by the currently-active `LibraryEntry` (matched from the current route, computed inside `LibraryPageShell`, no input required from the consuming page): the library's real published version (`LibraryEntry.version`, sourced by hand from that library's `package.json`, same "no invented version numbers" rule `libraries.ts` already documents for name/description), the install command (`npm i <importPath>`), and two real links: npm (`npmjs.com/package/<importPath>`) and GitHub (`github.com/zhunam/zhunam-ng-libs/tree/master/libs/<dir>`, the library's real subdirectory, not the repo root). Distinct from, and never a replacement for, the existing "Usage" panel at the bottom of each demo page (a longer code snippet showing the component in use) — this card is a quick reference sitting in the navigation itself, the Usage panel stays exactly where it already was.
